...
Harp is built so another school can adopt it without rebuilding anything. Almost everything event-specific — the hackathon's name, dates, contact details, application form, scan types, FAQs, sponsors, and schedule — is runtime configuration managed from the super-admin UI, not code.

{/* Diagram: the upstream/fork relationship — a box for hackutd/harp (upstream) with release tags flowing down into yourschool/harp (fork) via `git merge`, and beside it a separate yourschool marketing site built from the marketing template, connected to the fork's API by the public content API. */}
...
2. **Deploy it.** Stand up the Go API + portal, PostgreSQL, and SuperTokens, plus the optional services you want (GCS, email provider, web push, Apple Wallet). See [Deployment](/harp/deployment/production).
...
<Note>
  Adoption tooling is actively being improved — a standalone marketing template repo, a committed `.env.example`, and a dedicated portal branding directory are in progress. This guide will be updated as they land.
</Note>
