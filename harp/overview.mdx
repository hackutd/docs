...
Harp (Hacker Applications & Review Platform) is HackUTD's open-source platform for running a hackathon end to end. It handles everything from the moment applications open to the final day of the event: hacker applications, admin review workflows, acceptance decisions, check-ins, meals, schedules, and public event content.

Harp is built to be adopted. Any school can run it for their own hackathon — the event's name, dates, application form, scan types, FAQs, sponsors, and schedule are all runtime settings, not code.
