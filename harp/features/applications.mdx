...
Application sections, fields, and validation rules are stored as a dynamic schema, so each event designs its own form from the admin UI. Changing a question doesn't require a code change or redeploy.
...
On event day, a FIFO **walk-in queue** manages unregistered attendees and promotes them into the event as capacity allows.
