...
{/* Diagram: a role-hierarchy diagram (hacker, admin, super admin) with each role annotated with its permission scope. Hackers manage their own application and event tools; admins handle review, scans, and content with granular permissions; super admins own identity settings, roles, and the reset. */}
