...
| Variable | Purpose |
| --- | --- |
| `HACKATHON_NAME` | **Bootstrap default only.** Once a super admin completes onboarding, the name comes from the database setting — renaming the event needs no redeploy |
...
| Variable | Purpose |
| --- | --- |
| `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` | Web push keys — generate with `task gen-vapid` |
